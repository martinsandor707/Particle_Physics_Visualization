...
_AGGREGATES = """
    ...
    coalesce(sum(n_correct_abs), 0)             AS n_correct,
    coalesce(sum(n_true_a_abs), 0)              AS n_true_a,
    coalesce(sum(n_pred_a_abs), 0)              AS n_pred_a,
    coalesce(sum(n_tp_abs), 0)                  AS n_tp,
    coalesce(sum(n_split_cells), 0)              AS n_shared,
    coalesce(sum(sum_abs_err_abs), 0.0)         AS sum_abs_err,
    coalesce(sum(sum_wabs_err_abs), 0.0)        AS sum_wabs_err,
    coalesce(sum(sum_sq_err_abs), 0.0)          AS sum_sq_err,

    -- Energy residuals are per-event quantities, so they are averaged over
    -- events here rather than over voxels.
    avg(e_a_pred_abs - e_a_true_abs)                                        AS bias_a,
    sqrt(avg(pow(e_a_pred_abs - e_a_true_abs, 2)))                          AS rmse_a,
    avg((e_a_pred_abs - e_a_true_abs) / nullif(e_a_true_abs, 0))                AS rel_bias_a,
    stddev_samp((e_a_pred_abs - e_a_true_abs) / nullif(e_a_true_abs, 0))        AS rel_res_a,
    median(abs(e_a_pred_abs - e_a_true_abs) / nullif(e_a_true_abs, 0))          AS rel_med_a,
    corr(e_a_pred_abs, e_a_true_abs)                                        AS corr_a,
    regr_slope(e_a_pred_abs, e_a_true_abs)                                  AS slope_a,

    avg(e_b_pred_abs - e_b_true_abs)                                        AS bias_b,
    sqrt(avg(pow(e_b_pred_abs - e_b_true_abs, 2)))                          AS rmse_b,
    avg((e_b_pred_abs - e_b_true_abs) / nullif(e_b_true_abs, 0))                AS rel_bias_b,
    stddev_samp((e_b_pred_abs - e_b_true_abs) / nullif(e_b_true_abs, 0))        AS rel_res_b,
    median(abs(e_b_pred_abs - e_b_true_abs) / nullif(e_b_true_abs, 0))          AS rel_med_b,
    corr(e_b_pred_abs, e_b_true_abs)                                        AS corr_b,
    regr_slope(e_b_pred_abs, e_b_true_abs)                                  AS slope_b
"""
